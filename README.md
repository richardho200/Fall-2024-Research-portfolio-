# Fall-2026-Research-portfolio
Computional Media PhD UCSC and Future PhD Admission Committee may refer to this repos 

Refer to the Fall 2026 SOP for furher explaination on the generative AI tutor project

The first project: https://github.com/richardho200/Fall-2024-Research-portfolio-/tree/main/GUIAITutor

This project showcases the functionality of generative AI gives detailed feedback on sample undergraduate questions from CSU San Marcos in the GUI Python program. More details will be given in the github repos. 

Youtube Demo for the remote server demonstration: https://youtu.be/8hbi-ZGbwfE

YT Demo is the formal explaination of the program in this repos: https://youtu.be/SiTCJabX8oA


2nd Project is the Initial study of generative AI tutorbot on analysis of respective version of GPT: https://github.com/richardho200/AI-Tutor-Bot
